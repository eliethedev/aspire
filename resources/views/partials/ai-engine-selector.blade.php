...
<div class="mb-3 rounded-lg border border-indigo-100 dark:border-indigo-900/40 bg-indigo-50/60 dark:bg-indigo-900/10 px-3 py-2.5">
    <div class="flex flex-wrap items-center gap-x-3 gap-y-2">
        <span id="ai-engine-badge" class="{{ $engineMeta ? '' : 'hidden' }} inline-flex items-center gap-1.5 text-[11px] font-semibold text-indigo-700 dark:text-indigo-300">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
            <span id="ai-engine-badge-text">{{ $engineMeta['label'] ?? '' }}{{ isset($engineMeta['model']) ? ' · '.$engineMeta['model'] : '' }}{{ !empty($engineMeta['fallback_used']) ? ' · baseline failover' : '' }}{{ !empty($engineMeta['manual']) ? ' · manual' : '' }}</span>
        </span>
