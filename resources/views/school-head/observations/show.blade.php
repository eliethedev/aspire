...
@if(session('success'))
    <div class="max-w-7xl mx-auto px-6 mb-4">
        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 text-green-800 rounded-xl px-5 py-3 text-sm flex items-center gap-2">
...
@if(session('error'))
    <div class="max-w-7xl mx-auto px-6 mb-4">
        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 text-red-800 dark:text-red-300 rounded-xl px-5 py-3 text-sm flex items-center gap-2">
...
<div class="max-w-7xl mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Leadership Observation</h1>
                @if($observation->status === 'cancelled')
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400">
                        <span class="w-1.5 h-1.5 rounded-full bg-red-50 dark:bg-red-900/200"></span> Cancelled
                    </span>
                @endif
            </div>
            ...
            @if($observation->subject)
                <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">{{ $observation->subject }}</p>
            @endif
        </div>
        <a href="{{ route('school-head.observations.index') }}"
           class="px-6 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-800 transition-colors text-sm">
            Back to List
        </a>
    </div>
        ...
        <div class="flex items-center justify-between">
            @foreach($stageKeys as $i => $key)
                ...
                <div class="flex items-center">
                    <div class="flex items-center justify-center w-10 h-10 rounded-full {{ $done ? 'bg-green-600 text-white' : ($active ? 'bg-indigo-600 text-white ring-2 ring-indigo-200' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400') }} font-semibold text-sm">
                        @if($done)
    ...
    <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-6 mb-6" x-data="{ showRejectModal: false }">
            ...
            <div class="flex-1">
                ...
                <div class="flex items-center gap-3 mt-4">
                    ...
                    <button @click="showRejectModal = true"
                            class="px-5 py-2.5 border border-red-300 text-red-700 dark:text-red-400 rounded-lg text-sm font-medium hover:bg-red-50 dark:bg-red-900/20 transition-colors">
                        Request Reschedule
                    </button>
                </div>
            </div>
        ...
        <div x-show="showRejectModal"
            ...
            <div class="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-lg w-full mx-4 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button @click="showRejectModal = false" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-400">
                ...
                <form action="{{ route('school-head.observations.reject', $observation) }}" method="POST"
                    ...
                    <div class="flex items-center justify-end gap-3 mt-6">
                        <button type="button" @click="showRejectModal = false"
                                class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-50 dark:bg-gray-800 transition-colors">Cancel</button>
                        <button type="submit"
                                :disabled="submitting"
    ...
    <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 rounded-xl p-6 mb-6">
        <div class="flex gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-emerald-700 dark:text-emerald-400 mt-1">You confirmed this observation on {{ $observation->confirmed_at?->format('M d, Y \a\t h:i A') }}.</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-xl p-6 mb-6">
        <div class="flex gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-red-700 dark:text-red-400 mt-1">You rejected this observation on {{ $observation->rejected_at?->format('M d, Y \a\t h:i A') }}.</p>
                @if($observation->rejection_reason)
                <p class="text-sm text-red-700 dark:text-red-400 mt-1"><strong>Reason:</strong> {{ str_replace('_', ' ', ucwords($observation->rejection_reason)) }}</p>
                @endif
                @if($observation->rejection_notes)
                <p class="text-sm text-red-700 dark:text-red-400 mt-1"><strong>Notes:</strong> {{ $observation->rejection_notes }}</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-xl p-6 mb-6">
        <div class="flex gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-red-700 dark:text-red-400 mt-1">This observation was cancelled on {{ $observation->cancelled_at?->format('M d, Y \a\t h:i A') }}.</p>
                @if($observation->cancellation_reason)
                <p class="text-sm text-red-700 dark:text-red-400 mt-1"><strong>Reason:</strong> {{ ucwords(str_replace('_', ' ', $observation->cancellation_reason)) }}</p>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Leadership plan, AI insights, and supervisor recommendations</p>
                </div>
            ...
            @if($observation->stage === 'pre_observation_planning')
                <div class="mb-6 p-5 border-2 border-dashed border-indigo-200 rounded-xl bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-indigo-900 dark:text-indigo-300">Upload Your Leadership Plan</h3>
                            <p class="text-xs text-indigo-600 dark:text-indigo-400">Submit your leadership or management plan for supervisor review</p>
                        </div>
                    </div>
                    <form action="{{ route('school-head.observations.upload-plan', $observation) }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                            <select name="plan_type" required
                                    class="px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-white">
                            ...
                            <input type="file" name="plan_file" id="plan_file" accept=".pdf,.doc,.docx,.pptx,.xlsx"
                                   class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-100 file:text-indigo-700 dark:text-indigo-300 hover:file:bg-indigo-200 transition-colors cursor-pointer">
                            <button type="submit"
                                    :disabled="submitting"
                        ...
                        @error('plan_type')
                            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                        @enderror
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Accepted formats: PDF, DOC, DOCX, PPTX, XLSX (max 20MB)</p>
                    </form>
                </div>
            @endif
            ...
            @if($observation->preObservationPlanning)
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20 border border-emerald-100 dark:border-emerald-800">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-white dark:bg-gray-900 shadow-sm flex items-center justify-center shrink-0">
                        ...
                        <a href="{{ asset('storage/' . $observation->preObservationPlanning->lesson_plan_file) }}" target="_blank"
                           class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl hover:bg-indigo-100 transition-colors">
                    ...
                    <div class="p-4 rounded-xl bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border border-purple-100 dark:border-purple-800">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-2 h-2 rounded-full bg-purple-50 dark:bg-purple-900/200"></div>
            ...
            @else
                @if($observation->stage !== 'pre_observation_planning')
                    <div class="flex flex-col items-center py-8 text-center">
                        <svg class="w-10 h-10 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Pre-observation discussion between school head and supervisor</p>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">Performance assessment</p>
                    </div>
                </div>
                <div class="text-right">
                    ...
                    @php
                        ...
                        $scoreColor = $scorePct >= 80 ? 'text-emerald-600 dark:text-emerald-400' : ($scorePct >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400');
            ...
            <div class="space-y-2.5">
                @foreach($observation->cotRatings as $rating)
                    @php
                        ...
                        $rPct = $r ? ($r / 6) * 100 : 0;
                        $rColor = !$r ? 'bg-gray-100 border-gray-200' : ($r >= 5 ? 'border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20' : ($r >= 4 ? 'border-blue-200 dark:border-blue-700 bg-blue-50 dark:bg-blue-900/20' : ($r >= 3 ? 'border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20' : 'border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-900/20')));
                        $rBadge = !$r ? 'bg-gray-100 text-gray-500 dark:text-gray-400' : ($r >= 5 ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : ($r >= 4 ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : ($r >= 3 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400')));
                    @endphp
