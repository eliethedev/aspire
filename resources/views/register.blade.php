...
<body class="antialiased font-sans dark-bg min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md glass-card rounded-2xl p-8 md:p-10 relative z-10">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-5">
            ...
            <!-- Role Field -->
            <div>
                <label class="block text-sm font-semibold text-white/80 mb-2">Role</label>
                <select name="role" id="role" class="dark-input w-full px-4 py-3 rounded-xl" required>
                    <option value="" class="bg-[#1a1a2e]">Select Role</option>
                    <option value="teacher" {{ old('role') == 'teacher' ? 'selected' : '' }} class="bg-[#1a1a2e]">Teacher</option>
                    <option value="supervisor" {{ old('role') == 'supervisor' ? 'selected' : '' }} class="bg-[#1a1a2e]">Supervisor</option>
                    <option value="school_head" {{ old('role') == 'school_head' ? 'selected' : '' }} class="bg-[#1a1a2e]">School Head</option>
                </select>
                @error('role')
                    <p class="mt-2 text-sm text-rose-400 flex items-center">
                        <svg class="w-4 h-4 mr-1.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
