...
<div class="max-w-7xl mx-auto px-6">
        ...
        <div class="flex items-center justify-between">
            @foreach($stageKeys as $i => $key)
                ...
                <div class="flex items-center {{ $active ? 'cursor-default' : '' }}">
                    ...
                    <span class="ml-2 {{ $done ? 'text-gray-600 font-medium' : ($active ? 'text-indigo-600 font-medium' : 'text-gray-400') }} text-sm">{{ $stageLabels[$key] }}</span>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-8">
        @foreach($stageKeys as $i => $key)
            ...
            <div class="bg-white rounded-xl border {{ $active ? 'border-indigo-300 ring-2 ring-indigo-100' : 'border-gray-100' }} shadow-sm p-4 stage-card">
                <div class="flex items-center gap-3 mb-2">
                    <div class="w-9 h-9 rounded-lg {{ $done ? 'bg-green-100 text-green-700' : ($active ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-50 text-gray-400') }} flex items-center justify-center">
                        {!! $icon !!}
                    </div>
                    <span class="text-xs font-semibold {{ $done ? 'text-green-600' : ($active ? 'text-indigo-600' : 'text-gray-400') }} uppercase tracking-wide">
                        {{ $done ? 'Completed' : ($active ? 'Current' : ($i < $currentIdx ? 'Available' : 'Locked')) }}
                    </span>
                </div>
                <h4 class="font-semibold text-gray-900 text-sm mb-0.5">{{ $stageLabels[$key] }}</h4>
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="font-semibold text-gray-900">{{ $observation->observer?->user?->name ?? $observation->observer?->name ?? 'Unknown' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <p class="text-gray-900 mt-1">{{ is_array($observation->preObservationPlanning->ai_insights) ? json_encode($observation->preObservationPlanning->ai_insights) : $observation->preObservationPlanning->ai_insights }}</p>
                </div>
                @endif
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-amber-50 border border-amber-100">
                    <span class="text-xs font-semibold text-amber-700 uppercase tracking-wider">Discussion Notes</span>
                    <p class="text-gray-900 mt-1">{{ $observation->preConference->discussion_notes }}</p>
                </div>
                @endif
                @if($observation->preConference->finalized_focus)
                <div class="md:col-span-2 p-4 rounded-xl bg-blue-50 border border-blue-100">
                    <span class="text-xs font-semibold text-blue-700 uppercase tracking-wider">Finalized Focus</span>
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-emerald-50 border border-emerald-100">
                    <span class="text-xs font-semibold text-emerald-700 uppercase tracking-wider">Teacher Reflection</span>
