...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">Monitor all classroom observations and evaluations across the system.</p>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('admin.observations.index') }}">
            <div class="flex flex-wrap items-end gap-3">
                <div class="flex-1 min-w-[200px]">
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-1.5">Search</label>
                    <div class="relative">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-1.5">Type</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-1.5">Status</label>
                    <select name="status"
                            class="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-1.5">Stage</label>
                    <select name="stage"
                            class="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                ...
                @if(request()->anyFilled(['search', 'observation_type', 'status', 'stage']))
                    <a href="{{ route('admin.observations.index') }}"
                       class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-300 transition-colors">
                        Clear
                    </a>
                @endif
            </div>
    ...
    @forelse($observations as $observation)
        ...
        <div class="obs-card bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5 mb-4">
            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium {{ $isTeacher ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' }}">
                ...
                <div class="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 shrink-0">
                ...
                <div class="flex items-center gap-2 text-xs">
                    ...
                    @foreach($stages as $i => $s)
                        ...
                        <div class="flex items-center gap-2">
                            <div class="flex items-center gap-1.5">
                                <div class="stage-dot {{ $done ? 'bg-indigo-50 dark:bg-indigo-900/200' : ($active ? 'bg-indigo-400 ring-2 ring-indigo-100' : 'bg-gray-200') }}"></div>
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    @else
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium
                            ...
                            <span class="w-1.5 h-1.5 rounded-full {{ $observation->status === 'completed' ? 'bg-green-50 dark:bg-green-900/200' : ($observation->status === 'scheduled' ? 'bg-amber-50 dark:bg-amber-900/200' : 'bg-blue-50 dark:bg-blue-900/200') }}"></span>
                            {{ ucwords(str_replace('_', ' ', $observation->status)) }}
                        </span>
                    @endif

                    <a href="{{ route('admin.observations.show', $observation) }}"
                       class="px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:bg-gray-800 rounded-lg transition-colors">
                        View Details
                    </a>
                </div>
            </div>
    ...
    @empty
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">No observations found</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-6">There are no observations matching your criteria.</p>
            <a href="{{ route('admin.observations.index') }}"
