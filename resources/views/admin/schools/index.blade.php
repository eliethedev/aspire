    ...
    <div class="bg-white rounded-xl shadow-sm glass-card p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-500 mt-1">Manage educational institutions and their settings.</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm glass-card p-6">
        <form method="GET" action="{{ route('admin.schools.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="search" name="search" 
                           class="w-full px-3 py-2 border text-gray-900 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div class="flex items-center mt-2">
                <a href="{{ route('admin.schools.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800">
                    Clear filters
                </a>
            </div>
            @endif
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 text-gray-900">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500">{{ $school->domain ?? 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500">{{ $school->subdomain ?? 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500">{{ $school->users_count ?? 0 }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500">{{ $school->created_at->format('M j, Y') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center space-x-3">
                                <a href="{{ route('admin.schools.show', $school) }}" 
                                   class="text-indigo-600 hover:text-indigo-900 text-sm font-medium" title="View">
                                    View
                                </a>
                                <a href="{{ route('admin.schools.edit', $school) }}" 
                                   class="text-blue-600 hover:text-blue-900 text-sm font-medium" title="Edit">
                                    Edit
                                </a>
                                <form method="POST" 
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-900 text-sm font-medium" title="Delete">
                                        Delete
                                    </button>
                                </form>
                            </div>
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            ...
                            <p class="mt-2 text-sm text-gray-500">No schools found.</p>
                            <a href="{{ route('admin.schools.create') }}" class="mt-3 inline-flex items-center text-sm text-indigo-600 hover:text-indigo-500">
