...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    <div class="flex items-center justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-gray-500 dark:text-gray-400 mt-1">Complete audit trail of all system activities. Append-only â€” logs cannot be edited or deleted.</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5 mb-6">
        <form method="GET" action="{{ route('admin.audit-logs.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">User</label>
                    <select name="user_id"
                            class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Module</label>
                    <select name="module"
                            class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
            ...
            <div class="flex items-center gap-3">
                ...
                @if(request()->anyFilled(['search', 'user_id', 'role', 'module', 'action', 'status', 'date_from', 'date_to']))
                    <a href="{{ route('admin.audit-logs.index') }}"
                       class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 transition-colors">
                        Clear
                    </a>
                @endif
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider">Timestamp</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider">User</th>
                        ...
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider">Module</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider">Record</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider">Status</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400 text-xs uppercase tracking-wider">IP Address</th>
                        ...
                        <tr class="log-entry">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-gray-900 dark:text-gray-100 font-medium text-sm">{{ $log->user?->name ?? 'System' }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                @if($log->role)
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-400 dark:text-gray-500 font-mono">
                                {{ $log->ip_address ?? 'â€”' }}
                            </td>
