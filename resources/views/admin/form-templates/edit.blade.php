...
<div class="max-w-6xl mx-auto px-4 space-y-6">
    <nav class="text-sm">
        <ol class="flex items-center gap-2 text-gray-500">
            ...
            <li class="text-gray-900 font-medium">{{ $formTemplate->name }}</li>
        </ol>
    </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 sticky top-6">
                    <h2 class="text-lg font-semibold text-gray-900 mb-4">Template Settings</h2>
                    <div class="space-y-4">
                        <div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="w-full px-3 py-2 rounded-lg border border-gray-300 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">{{ old('description', $formTemplate->description) }}</textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-400">Assign to a specific observation type, or "All Types".</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                            <div class="flex items-center gap-2">
                                @if($formTemplate->is_active)
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">Active</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Inactive</span>
                                @endif
                                <span class="text-xs text-gray-500">v{{ $formTemplate->version }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        <h3 class="text-sm font-semibold text-gray-700 mb-2">Duplicate Template</h3>
                        <p class="text-xs text-gray-500 mb-3">Copy this template to a new school year.</p>
                        <form method="POST" action="{{ route('admin.form-templates.duplicate', $formTemplate) }}" class="flex gap-2" onsubmit="return confirm('Duplicate this template?')">
                            ...
                            <select name="school_year" required
                                    class="flex-1 px-2 py-1.5 rounded-lg border border-gray-300 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                @foreach($schoolYears as $value => $label)
                                    <option value="{{ $value }}" {{ $value == $formTemplate->school_year ? '' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            <button type="submit"
                                    class="px-3 py-1.5 text-sm font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors whitespace-nowrap">Duplicate</button>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                <template x-for="(section, sIdx) in sections" :key="sIdx">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <!-- Section Header -->
                        <div class="flex items-center gap-3 p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                            <span class="section-handle text-gray-400">
                            ...
                            <div class="flex-1 grid grid-cols-2 gap-3">
                                <div>
                                    <label class="text-xs text-gray-500">Section Key</label>
                                    <input type="text" x-model="section.key" :name="`sections[${sIdx}][key]`"
                                           :readonly="!!section.id"
                                           :class="section.id ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white text-gray-900'"
                            ...
                            <button type="button" @click="removeSection(sIdx)" class="text-red-400 hover:text-red-600 p-1">
                        ...
                        <div class="p-4 space-y-3">
                            <template x-for="(field, fIdx) in section.fields" :key="fIdx">
                                <div class="field-item border border-gray-200 rounded-lg p-3 bg-white">
                                    <div class="flex items-start gap-3">
                                        <span class="field-handle text-gray-400 mt-1">
                                        ...
                                        <div class="flex-1 space-y-2">
                                            ...
                                            <div class="grid grid-cols-2 gap-2" x-show="!['heading','paragraph','hr'].includes(field.type)">
                                                ...
                                                <div>
                                                    <label class="text-xs text-gray-500">Help Text</label>
                                            ...
                                            <div x-show="['select','radio','checkbox'].includes(field.type)">
                                                <label class="text-xs text-gray-500">Options <span class="text-gray-400">(one per line, or <code>value|Label</code>)</span></label>
                                            ...
                                            <div class="grid grid-cols-3 gap-2" x-show="!['heading','paragraph','hr'].includes(field.type)">
                                                <div>
                                                    <label class="text-xs text-gray-500">Validation Rules</label>
                                                ...
                                                <div>
                                                    <label class="text-xs text-gray-500">Column Map</label>
                                                ...
                                                <div class="flex items-end gap-2">
                                                    ...
                                                    <button type="button" @click="removeField(sIdx, fIdx)" class="text-red-400 hover:text-red-600 p-1 ml-auto">
                            ...
                            <button type="button" @click="addField(sIdx)"
                                    class="w-full py-2 border-2 border-dashed border-gray-200 text-gray-400 hover:border-indigo-300 hover:text-indigo-500 rounded-lg text-sm transition-colors">
                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                                Add Field
                            </button>
                        </div>
                ...
                <div x-show="sections.length === 0" class="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <p class="text-gray-500 font-medium">No sections yet.</p>
                    <p class="text-sm text-gray-400 mt-1">Click "Add Section" to start building your form.</p>
                </div>
            </div>
        </div>
    </form>
