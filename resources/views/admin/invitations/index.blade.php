...
<div class="max-w-7xl mx-auto px-6 space-y-8">
    ...
    <!-- Invitations Table -->
    <div class="bg-white rounded-xl shadow-sm border glass-card overflow-hidden">
        <table class="w-full">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                                    <span class="text-blue-600 font-medium">{{ substr($invitation->user->name, 0, 1) }}</span>
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $invitation->user->name }}</div>
                                    <div class="text-xs text-gray-500">Invited by {{ $invitation->invitedBy->name }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            @if(!$invitation->is_used && !$invitation->isExpired())
                                <form method="POST" action="{{ route('admin.invitations.resend', $invitation) }}" class="inline">
                                    @csrf
                                    <button type="submit" class="text-blue-600 hover:text-blue-900" title="Resend">
                            ...
                            @endif
                            @if($invitation->is_used)
                                <a href="{{ route('admin.users.show', $invitation->user) }}" class="text-green-600 hover:text-green-900" title="View User">
