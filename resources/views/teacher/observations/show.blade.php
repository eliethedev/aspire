        ...
        <div class="flex gap-3">
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-amber-700 mt-1">
                    Your supervisor has scheduled an observation on <strong>{{ $observation->observation_date->format('M d, Y') }}</strong>.
                ...
                <div class="flex items-center gap-3 mt-4">
                    ...
                    <button @click="showRejectModal = true"
                            class="px-5 py-2.5 border border-red-300 text-red-700 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors">
                        Request Reschedule
                    </button>
                </div>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            @if($observation->stage === 'pre_observation_planning')
                <div class="mb-6 p-5 border-2 border-dashed border-indigo-200 rounded-xl bg-gradient-to-br from-indigo-50 to-blue-50">
            ...
            @if($observation->preObservationPlanning)
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2 mt-0.5">
                                    ...
                                    <span class="text-xs text-gray-500">{{ preg_replace('/^\d+_/', '', basename($observation->preObservationPlanning->lesson_plan_file)) }}</span>
                                </div>
                            </div>
                        </div>
                        <a href="{{ asset('storage/' . $observation->preObservationPlanning->lesson_plan_file) }}" target="_blank"
                           class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors">
                    ...
                    <div class="p-4 rounded-xl bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-100">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-2 h-2 rounded-full bg-purple-500"></div>
                            <span class="text-xs font-semibold text-purple-700 uppercase tracking-wider">AI Insights</span>
                        </div>
                        <p class="text-sm text-gray-700 leading-relaxed">{{ $observation->preObservationPlanning->ai_insights }}</p>
                    </div>
                    @endif
                    ...
                    <div class="p-4 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-xs font-semibold text-blue-700 uppercase tracking-wider">Suggested Focus</span>
                        </div>
                        <p class="text-sm text-gray-700">{{ $observation->preObservationPlanning->suggested_focus }}</p>
                    </div>
                    @endif
                    ...
                    <div class="p-4 rounded-xl bg-gradient-to-br from-amber-50 to-yellow-50 border border-amber-100">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-xs font-semibold text-amber-700 uppercase tracking-wider">Supervisor Notes</span>
                        </div>
                        <p class="text-sm text-gray-700">{{ $observation->preObservationPlanning->supervisor_notes }}</p>
                    </div>
                    @endif
                </div>
            @else
                @if($observation->stage !== 'pre_observation_planning')
                    <div class="flex flex-col items-center py-8 text-center">
                        ...
                        <p class="text-sm text-gray-500">No pre-observation planning data recorded yet.</p>
                    </div>
                @endif
            @endif
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-gradient-to-br from-amber-50 to-yellow-50 border border-amber-100">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-sm font-semibold text-amber-800">Discussion Notes</span>
                    </div>
                    <p class="text-sm text-gray-700">{{ $observation->preConference->discussion_notes }}</p>
                </div>
                @endif
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="text-right">
                    <p class="text-xs text-gray-500 uppercase tracking-wider font-medium">Overall Score</p>
                    <div class="flex items-end gap-1">
                        <p class="text-gray-900 font-bold text-3xl tracking-tight">{{ number_format($observation->overall_score, 1) }}</p>
                        <p class="text-gray-400 font-medium text-lg mb-0.5">/ 6</p>
                    </div>
                    @php
            ...
            <div class="space-y-2.5">
                @foreach($observation->cotRatings as $rating)
                    @php
                        ...
                        $rPct = $r ? ($r / 6) * 100 : 0;
                        $rColor = !$r ? 'bg-gray-100 border-gray-200' : ($r >= 5 ? 'border-emerald-200 bg-emerald-50' : ($r >= 4 ? 'border-blue-200 bg-blue-50' : ($r >= 3 ? 'border-amber-200 bg-amber-50' : 'border-red-200 bg-red-50')));
                        $rBadge = !$r ? 'bg-gray-100 text-gray-500' : ($r >= 5 ? 'bg-emerald-100 text-emerald-700' : ($r >= 4 ? 'bg-blue-100 text-blue-700' : ($r >= 3 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700')));
                    @endphp
                    <div class="rounded-xl p-4 border {{ $rColor }}">
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-semibold text-gray-900">{{ $rating->indicator }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">{{ $rating->domain }}</p>
                                @if($rating->comments)
                                <p class="text-sm text-gray-600 mt-2 pt-2 border-t border-gray-200/60">{{ $rating->comments }}</p>
                                @endif
                            </div>
                            <div class="text-center shrink-0">
                                ...
                                <p class="text-[10px] text-gray-400 mt-0.5">/ 6</p>
                            </div>
                        </div>
                        @if($r)
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h2 class="text-lg font-semibold text-gray-900">Post-Conference</h2>
                    <p class="text-xs text-gray-500">Feedback and action plan from supervisor</p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="p-4 rounded-xl bg-gray-50 border border-gray-100">
                    <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Conference Date</span>
                    <p class="text-gray-900 font-medium mt-1">{{ $observation->postConference->conference_date->format('M d, Y') }}</p>
                </div>
                @endif
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-sm font-semibold text-emerald-800">Supervisor Feedback</span>
                    </div>
                    <p class="text-sm text-gray-700 leading-relaxed">{{ $observation->postConference->feedback }}</p>
                </div>
                @endif
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-100">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-sm font-semibold text-purple-800">AI Comparison (Plan vs Actual)</span>
                    </div>
                    <p class="text-sm text-gray-700 leading-relaxed">{{ $observation->postConference->ai_comparison }}</p>
                </div>
                @endif
                ...
                <div class="md:col-span-2 p-4 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-sm font-semibold text-blue-800">Action Plan</span>
                    </div>
                    <p class="text-sm text-gray-700 leading-relaxed">{{ $observation->postConference->action_plan }}</p>
                </div>
                @endif
            </div>
        </div>
