...
<div class="max-w-4xl mx-auto px-4 sm:px-6">
    ...
    <nav class="mb-6 text-sm">
        <ol class="flex items-center gap-2 text-gray-500 dark:text-gray-400">
            <li><a href="{{ route('teacher.feedback.index') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">Feedback & Coaching</a></li>
            ...
            <li class="text-gray-900 dark:text-gray-100 font-medium">Feedback Details</li>
        </ol>
    </nav>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-xs font-semibold text-green-600 dark:text-green-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
            ...
            <ul class="space-y-2">
                @foreach($feedback->strengths as $strength)
                    <li class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                        <svg class="w-4 h-4 text-green-500 dark:text-green-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4.5 12.75l6 6 9-13.5"/></svg>
                        {{ $strength }}
                    </li>
                @endforeach
            </ul>
        ...
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
            ...
            <ul class="space-y-2">
                @foreach($feedback->areas_for_improvement as $afi)
                    <li class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                        <svg class="w-4 h-4 text-amber-500 dark:text-amber-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        {{ $afi }}
                    </li>
                @endforeach
            </ul>
        ...
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
        ...
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">COT Ratings</h3>
            <div class="space-y-2">
                @foreach($feedback->observation->cotRatings as $rating)
                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{{ $rating->indicator }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $rating->domain }}</p>
                        </div>
                        <span class="shrink-0 ml-3 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                            {{ $rating->isNotObserved() ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' : ($rating->rating >= 4 ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : ($rating->rating >= 3 ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400')) }}">
        ...
        <!-- Published Info -->
        <div class="p-4 bg-gray-50 dark:bg-gray-800 flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
    ...
    <a href="{{ route('teacher.observations.show', $feedback->observation) }}"
       class="mt-4 inline-flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-medium">
