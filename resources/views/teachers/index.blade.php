    ...
    <div class="bg-white rounded-xl shadow-sm glass-card p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-white mt-1">Manage teacher profiles and assignments.</p>
            </div>
            <a href="{{ route('teachers.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm glass-card p-6">
        <form method="GET" action="{{ route('teachers.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label for="school_id" class="block text-sm font-medium text-white mb-1">School</label>
            ...
            <div class="flex items-center mt-2">
                <a href="{{ route('teachers.index') }}" class="text-sm text-blue-600 hover:text-blue-800">
                    Clear filters
                </a>
            </div>
            @endif
            ...
            <table class="w-full">
                <thead class="bg-dark border-glass-card">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">School</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Years of Service</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr class="hover:bg-dark">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-white">{{ $teacher->user->name }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-white">{{ $teacher->user->email }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm text-white">{{ $teacher->user->school->name ?? 'N/A' }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm text-white">{{ $teacher->department ?? 'N/A' }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm text-white">{{ $teacher->position ?? 'N/A' }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm text-white">{{ $teacher->years_of_service ?? 'N/A' }}</span>
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="text-white">
                                ...
                                <a href="{{ route('teachers.create') }}" class="mt-2 inline-flex text-blue-600 hover:text-blue-800">
