...
<div class="max-w-4xl mx-auto px-4 sm:px-6">
    <nav class="mb-6 text-sm">
        <ol class="flex items-center gap-2 text-gray-500">
            ...
            <li class="text-gray-900 font-medium">Edit Agreement</li>
        </ol>
    </nav>

    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">Edit Coaching Agreement</h1>
        <p class="text-gray-500 mt-1">{{ $agreement->observation->observee->user->name ?? 'Teacher' }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('supervisor.coaching.update', $agreement) }}" class="space-y-6"
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6" x-data="{ areas: {{ json_encode($agreement->focus_areas ?? ['']) }} }">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6" x-data="{ steps: {{ json_encode($agreement->action_steps ?? ['']) }} }">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <textarea name="resources_needed" rows="3"
                          class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ $agreement->resources_needed }}</textarea>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <textarea name="success_indicators" rows="3"
                          class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ $agreement->success_indicators }}</textarea>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <label class="block text-sm font-semibold text-gray-900 mb-2">Supervisor Notes</label>
                <textarea name="supervisor_notes" rows="3"
                          class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ $agreement->supervisor_notes }}</textarea>
            </div>
        </div>
