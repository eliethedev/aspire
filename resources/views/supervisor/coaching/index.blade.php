...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <nav class="mb-6 text-sm">
        <ol class="flex items-center gap-2 text-gray-500">
            <li><a href="{{ route('supervisor.dashboard') }}" class="hover:text-indigo-600 transition-colors">Dashboard</a></li>
            ...
            <li class="text-gray-900 font-medium">Coaching Agreements</li>
        </ol>
    </nav>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Coaching Agreements</h1>
            <p class="text-gray-500 mt-1">Create and manage coaching agreements for your teachers.</p>
        </div>
    </div>
    ...
    @if($agreements->isEmpty())
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 mb-1">No Coaching Agreements Yet</h3>
            <p class="text-sm text-gray-500 mb-6">Create a coaching agreement from a completed observation's post-conference.</p>
    ...
    @else
        <div class="space-y-4">
            @foreach($agreements as $agreement)
                <div class="agreement-card bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    <div class="p-5">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-gray-900">{{ $agreement->teacher?->user?->name ?? 'Teacher' }}</p>
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-3">
                            @foreach($agreement->focus_areas as $area)
                                <span class="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-600">{{ $area }}</span>
                            @endforeach
                        </div>
                        @endif
                        ...
                        <div class="flex items-center gap-4 mt-3 text-xs text-gray-500">
                            @if($agreement->teacher_signed_at)
                                <span class="flex items-center gap-1 text-green-600"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4.5 12.75l6 6 9-13.5"/></svg> Teacher Signed</span>
                            @endif
                            @if($agreement->supervisor_signed_at)
                                <span class="flex items-center gap-1 text-green-600"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4.5 12.75l6 6 9-13.5"/></svg> You Signed</span>
                            @endif
