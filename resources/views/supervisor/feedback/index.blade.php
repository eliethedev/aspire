...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <nav class="mb-6 text-sm">
        <ol class="flex items-center gap-2 text-gray-500">
            ...
            <li><svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z"/></svg></li>
            <li class="text-gray-900 font-medium">Feedback Management</li>
        </ol>
    </nav>
    ...
    <!-- Feedback Type Tabs -->
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-1.5 mb-6 inline-flex flex-wrap gap-1">
        @foreach($feedbackTypes as $key => $label)
            ...
            <a href="{{ route('supervisor.feedback.index', [$observation, 'type' => $key]) }}"
               class="tab-btn inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors
                   {{ $isActive ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' }}">
                {!! $icons[$key] !!}
                {{ $label }}
                @if($hasExisting)
                    <span class="inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold {{ $isActive ? 'bg-white text-indigo-600' : 'bg-indigo-100 text-indigo-600' }}">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
                    ...
                    <div class="flex flex-wrap items-center justify-center gap-3">
                        ...
                        <a href="{{ route('supervisor.feedback.create', [$observation, 'type' => $selectedType]) }}"
                           class="inline-flex items-center gap-2 px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition-colors">
            ...
            @else
                @foreach($typeFeedbacks as $feedback)
                    <div class="feedback-card bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                        <!-- Header -->
                        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold
                                    {{ $feedback->generated_by === 'ai' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('supervisor.feedback.edit', [$observation, $feedback]) }}"
                                   class="px-3 py-1.5 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg text-xs font-medium transition-colors">
                                    Edit
                                </a>
                                ...
                                <form method="POST" action="{{ route('supervisor.feedback.destroy', [$observation, $feedback]) }}" class="inline" onsubmit="return confirm('Delete this feedback?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            class="px-3 py-1.5 border border-red-200 text-red-600 hover:bg-red-50 rounded-lg text-xs font-medium transition-colors">
                                        Delete
                                    </button>
                                </form>
                            </div>
                        ...
                        <div class="p-6 space-y-5">
                            <!-- Analysis -->
                            <div>
                                <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Analysis</h4>
                            ...
                            <div>
                                <h4 class="text-xs font-semibold text-green-600 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                            ...
                            <div>
                                <h4 class="text-xs font-semibold text-amber-600 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                            ...
                            <div>
                                <h4 class="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Per-Indicator AI Feedback</h3>
                <div class="space-y-3">
                    @foreach($perIndicatorFeedbacks as $aiFb)
                        ...
                        <div class="border border-gray-200 rounded-lg p-4">
                            <div class="flex items-start justify-between mb-2">
                                <div>
                                    <p class="text-sm font-medium text-gray-900">{{ $rating->indicator }}</p>
                                    <p class="text-xs text-gray-500">{{ $rating->domain }}</p>
                                </div>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $rating->isNotObserved() ? 'bg-gray-100 text-gray-600' : ($rating->rating >= 4 ? 'bg-green-100 text-green-700' : ($rating->rating >= 3 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700')) }}">
                                    {{ $rating->isNotObserved() ? 'NO' : $rating->rating }}/6
                                </span>
                            </div>
                            @if($aiFb->analysis)
                                <p class="text-sm text-gray-700">{{ Str::limit($aiFb->analysis, 200) }}</p>
                            @endif
                            @if($aiFb->confidence_score)
                                <p class="text-xs text-gray-400 mt-1">Confidence: {{ number_format($aiFb->confidence_score * 100, 0) }}%</p>
                            @endif
                        </div>
                    @endforeach
                </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Quick Actions</h3>
                <div class="space-y-2.5">
                    <button type="button" onclick="openGenerateModal('{{ $selectedType }}')"
                            class="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-700 text-sm font-medium transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                        Generate AI Feedback
                    </button>
                    <a href="{{ route('supervisor.feedback.create', [$observation, 'type' => $selectedType]) }}"
                       class="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-medium transition-colors">
            ...
            <!-- Observation Info -->
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Observation Info</h3>
                <div class="space-y-3">
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-900">{{ $observation->subject ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500">Grade Level</span>
                        <p class="text-sm text-gray-900">{{ $observation->grade_level ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500">Date</span>
                        <p class="text-sm text-gray-900">{{ $observation->observation_date->format('M d, Y') }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500">Status</span>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $observation->status === 'completed' ? 'bg-green-100 text-green-700' : ($observation->status === 'in_progress' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700') }}">
            ...
            <!-- Stage Progress -->
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Stage Progress</h3>
                <div class="space-y-2.5">
                    ...
                    @foreach($stages as $i => $stage)
                        ...
                        <div class="flex items-center gap-2.5">
                            <div class="w-6 h-6 rounded-full flex items-center justify-center {{ $done ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400' }}">
                                @if($done)
                            ...
                            <span class="text-xs {{ $done ? 'text-gray-700 font-medium' : 'text-gray-400' }}">{{ $stageLabel }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <!-- Feedback Summary -->
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Feedback Summary</h3>
                <div class="space-y-2">
                    @foreach($feedbackTypes as $key => $label)
                        @php
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="text-xs {{ $publishedCount > 0 ? 'text-green-600 font-medium' : ($count > 0 ? 'text-amber-600' : 'text-gray-400') }}">
...
<div id="generate-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/30 backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 p-6">
        <div class="flex items-center gap-3 mb-4">
            ...
            <div>
                <h3 class="text-lg font-semibold text-gray-900">Generate AI Feedback</h3>
                <p id="generate-modal-type" class="text-sm text-gray-500"></p>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            <button type="button" onclick="closeGenerateModal()"
                    class="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium text-sm transition-colors">
                Cancel
            </button>
