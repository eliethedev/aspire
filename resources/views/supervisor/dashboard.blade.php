...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('supervisor.observations.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium">All Observations &rarr;</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Teachers</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Obs</p>
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ $stats['total_observations'] }}</p>
                </div>
            ...
            <div class="mt-3 flex items-center gap-2 text-xs">
                <span class="text-gray-500">{{ $stats['scheduled'] }} scheduled</span>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <div class="mt-3">
                ...
                <p class="text-xs text-gray-500 mt-1">{{ $stats['total_observations'] > 0 ? round(($stats['completed'] / $stats['total_observations']) * 100) : 0 }}% completion rate</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($stats['average_score'], 1) }}</p>
                </div>
            ...
            <div class="mt-3 flex items-center gap-1.5 text-xs">
                ...
                @elseif($trend < 0)
                    ...
                    <span class="text-gray-400">trend</span>
                @else
                    <span class="text-gray-400">No trend data</span>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ $stats['in_progress'] + $stats['stage_pre_planning'] + $stats['stage_pre_conference'] + $stats['stage_observation'] }}</p>
                </div>
                <div class="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center">
                    <svg class="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
            </div>
            <div class="mt-3 text-xs text-gray-500">{{ $stats['stage_post_conference'] }} in post-conference stage</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            @else
                <div class="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p class="text-sm text-gray-400">No scored observations yet to show trend data.</p>
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div class="flex items-center gap-2 mb-4">
                ...
                <h2 class="text-sm font-semibold text-gray-900 uppercase tracking-wider">Quick Actions</h2>
            </div>
            <div class="grid grid-cols-1 gap-3">
                <a href="{{ route('supervisor.observations.create') }}" class="flex items-center gap-3 p-3 rounded-lg bg-indigo-50 hover:bg-indigo-100 transition-colors group">
                    ...
                    <div>
                        <p class="text-sm font-medium text-indigo-700">New Observation</p>
                        <p class="text-xs text-indigo-500">Schedule a new classroom observation</p>
                    </div>
                </a>
                <a href="{{ route('supervisor.teachers.index') }}" class="flex items-center gap-3 p-3 rounded-lg bg-emerald-50 hover:bg-emerald-100 transition-colors group">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-emerald-500">Manage your teachers</p>
                    </div>
                </a>
                <a href="{{ route('supervisor.reports.index') }}" class="flex items-center gap-3 p-3 rounded-lg bg-purple-50 hover:bg-purple-100 transition-colors group">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-purple-500">Generate observation reports</p>
                    </div>
                </a>
                <a href="{{ route('supervisor.feedback.center') }}" class="flex items-center gap-3 p-3 rounded-lg bg-amber-50 hover:bg-amber-100 transition-colors group">
                    ...
                    <div>
                        <p class="text-sm font-medium text-amber-700">Feedback Center</p>
                        <p class="text-xs text-amber-500">Review AI feedback</p>
                    </div>
                </a>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
            ...
            @if($recentObservations->count() > 0)
                <a href="{{ route('supervisor.observations.index') }}" class="text-xs text-indigo-600 hover:text-indigo-800 font-medium">View All &rarr;</a>
            @endif
        </div>
        @if($recentObservations->count() > 0)
                    ...
                    <div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ $observation->observation_date->format('M d, Y') }} &middot; {{ $observation->subject ?? 'N/A' }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ route('supervisor.observations.show', $observation) }}" class="text-xs text-indigo-600 hover:text-indigo-800 font-medium">View</a>
                        </div>
                    </div>
                @endforeach
            </div>
        @else
            <div class="flex flex-col items-center py-12 text-center">
                ...
                <p class="text-sm text-gray-400">No recent activity to display.</p>
                <a href="{{ route('supervisor.observations.create') }}" class="mt-3 text-sm text-indigo-600 hover:text-indigo-800 font-medium">Create your first observation &rarr;</a>
            </div>
        @endif
    </div>
</div>
