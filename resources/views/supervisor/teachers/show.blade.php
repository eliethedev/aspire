    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('supervisor.observations.teacher-history', ['observeeId' => $teacher->id, 'type' => 'App\\Models\\Teacher']) }}"
               class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 text-dark-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <div class="flex flex-col items-center text-center mb-6">
                    <div class="w-20 h-20 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-3xl font-bold mb-3">
                        {{ strtoupper(substr($teacher->user->name, 0, 1)) }}
                    </div>
                    <h2 class="text-lg font-bold text-dark-900">{{ $teacher->user->name }}</h2>
                    <p class="text-sm text-dark-500">{{ $teacher->user->email }}</p>
                    @if($teacher->position)
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700 mt-2">
                            {{ $teacher->position }}
                        </span>
                    @endif
                </div>
        ...
        <div class="lg:col-span-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                    <p class="text-2xl font-bold text-indigo-600">{{ $stats['avg_score'] ? number_format($stats['avg_score'], 2) : 'N/A' }}</p>
                    ...
                    <div class="flex items-center justify-between py-3 {{ !$loop->last ? 'border-b border-gray-50' : '' }}">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="text-center shrink-0">
                                ...
                                <p class="text-lg font-bold text-indigo-600 leading-tight">{{ $observation->observation_date->format('d') }}</p>
                            </div>
                            <div class="min-w-0">
                        ...
                        <a href="{{ route('supervisor.observations.show', $observation) }}"
                           class="px-3 py-1.5 text-sm font-medium text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors shrink-0">
                            View
                        </a>
                    </div>
                @empty
