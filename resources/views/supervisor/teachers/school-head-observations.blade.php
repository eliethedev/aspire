...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <a href="{{ route('supervisor.school-heads.index') }}" class="inline-flex items-center gap-1.5 text-sm text-indigo-500 hover:text-indigo-600 mb-2 transition-colors">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-4">
            <div class="flex items-start justify-between">
                <div class="flex items-start gap-4">
                    <div class="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-sm font-bold shrink-0 mt-0.5">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium
                                @switch($obs->stage)
                                    @case('pre_observation_planning') bg-purple-50 text-purple-700 @break
                                    @case('pre_conference') bg-indigo-50 text-indigo-700 @break
                            ...
                            @elseif($obs->confirmation_status === 'rejected')
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-red-50 text-red-700">
                                    Rejected
                                </span>
                            @endif
                        </div>
                ...
                <a href="{{ route('supervisor.observations.show', $obs) }}"
                   class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors">
                    View Details
