...
<div class="max-w-7xl mx-auto px-4 sm:px-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $observeeName }}</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">Complete observation history</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
            <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $stats['avg_score'] ? number_format($stats['avg_score'], 2) : 'N/A' }}</p>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex items-center gap-4">
                    <div class="text-center">
                        <p class="text-lg font-bold text-gray-900 dark:text-gray-100">{{ $observation->observation_date->format('M') }}</p>
                    ...
                    <div>
                        ...
                        @if($observation->overall_score)
                            <p class="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Score: {{ number_format($observation->overall_score, 2) }} / 6.00</p>
                        @endif
                    </div>
                </div>
                <div class="flex items-center gap-2 shrink-0">
                    <a href="{{ route('supervisor.observations.show', $observation) }}"
                       class="px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-50 dark:bg-gray-800 rounded-lg transition-colors">
                        View Details
                    </a>
                    @php
    ...
    @empty
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-12 text-center">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">This teacher has no observation records yet.</p>
        </div>
    @endforelse
