                    ...
                    <div class="p-5">
                        @if($observation->postConference)
                            <div class="space-y-3">
                                ...
                                @if($observation->postConference->ai_comparison && !$observation->isSchoolHeadObservation())<div class="rounded-xl bg-indigo-50 border border-indigo-100 p-4"><p class="text-xs font-semibold tracking-widest uppercase text-indigo-700 mb-1">AI Comparison (Plan vs Actual)</p><p class="text-sm">{{ $observation->postConference->ai_comparison }}</p></div>@endif
                                @if($observation->postConference->feedback && !$observation->isSchoolHeadObservation())<div class="rounded-xl bg-gray-50 border p-4"><p class="text-xs font-semibold tracking-widest uppercase text-gray-400 mb-1">Feedback</p><p class="text-sm whitespace-pre-wrap">{{ $observation->postConference->feedback }}</p></div>@endif
