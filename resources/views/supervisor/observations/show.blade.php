...
<div class="obs-show mock-wrap max-w-7xl mx-auto px-1 py-1" x-data="{ detailFilter: '{{ $initialFilter }}' }">
    ...
    <div class="mock-topbar">
        <div class="mock-crumbs">Supervisor <span>/</span> <b>Review details</b></div>
    ...
    {{-- Hero Card --}}
    <div class="hero-card rounded-2xl p-4 sm:p-4 shadow-sm mb-4">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2 text-sm">
                        ...
                        @if($observation->isTeacherObservation() && $observation->subject)
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-300 text-xs font-medium">{{ $observation->subject }}@if($observation->grade_level) · {{ $observation->grade_level_label }}@endif</span>
                        @endif
                    ...
                    <div class="mt-3 flex flex-wrap gap-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <span>Step: <strong class="text-gray-700 dark:text-gray-200">{{ $stageConfig[$observation->stage]['label'] ?? ucfirst(str_replace('_',' ', $observation->stage)) }}</strong></span>
    ...
    {{-- Sticky Filter Bar --}}
    <div class="sticky top-[65px] z-20 -mx-4 sm:mx-0 sm:rounded-2xl sticky-filter bg-white/95 dark:bg-gray-900/95 border-y sm:border border-gray-200 dark:border-gray-700 shadow-sm mb-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        ...
        <div class="lg:col-span-8 space-y-4 min-w-0">
            ...
            @if($observation->status === 'cancelled')
            <div class="rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 p-4 flex gap-3">
                ...
                <div><h3 class="font-semibold text-red-800 dark:text-red-300">Review cancelled</h3><p class="text-sm text-red-700 dark:text-red-300">Cancelled on {{ $observation->cancelled_at?->format('M d, Y \a\t h:i A') }} by {{ $observation->cancelledBy?->name ?? 'Unknown' }} · {{ ucwords(str_replace('_',' ', $observation->cancellation_reason ?? '')) }}</p></div>
            </div>
            @endif

            {{-- Stepper Cards --}}
            <section class="mock-panel" aria-label="Review progress">
                <div class="mock-panel-head"><h2>Review progress</h2><span class="hint">{{ $observation->status === 'completed' ? 'Finished' : 'Now: ' . ($stageConfig[$observation->stage]['label'] ?? '') }}</span></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" style="padding:14px 16px">
                @foreach($stageKeys as $i => $key)
                    ...
                    @else
                        <div class="flex items-center gap-3 p-3.5 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-dashed border-gray-200 dark:border-gray-700 opacity-70">
                            ...
                            <span class="min-w-0"><span class="block text-xs font-semibold tracking-widest uppercase text-gray-400 dark:text-gray-500">Queued</span><span class="block text-sm font-semibold text-gray-500 dark:text-gray-400 truncate">{{ $stageLabels[$key] }}</span></span>
            ...
            <div class="space-y-4">
                ...
                <div x-show="detailFilter==='all' || detailFilter==='pre_observation'" x-transition.opacity class="section-card page-card overflow-hidden">
                    <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0"><h2 class="font-semibold text-gray-900 dark:text-white">Lesson planning</h2><p class="text-xs text-gray-500 dark:text-gray-400">Lesson plan and goals</p></div>
                    ...
                    <div class="p-4">
                        ...
                        @else
                            <div class="text-center py-8"><div class="w-12 h-12 rounded-2xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center mx-auto mb-3"></div><p class="text-sm font-medium text-gray-600 dark:text-gray-400">Nothing here yet</p><p class="text-xs text-gray-400 dark:text-gray-500">Finish the lesson planning step to fill this in.</p></div>
                        @endif
                    </div>
                </div>
                ...
                <div class="section-card rounded-2xl border-2 border-dashed border-violet-200 dark:border-violet-500/20 bg-violet-50/40 dark:bg-violet-500/10 p-8 text-center">
                    ...
                    <h3 class="font-semibold text-gray-900 dark:text-white">Final chat not done yet</h3><p class="text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-md mx-auto">Review the school head's feedback chat.</p>
                    <a href="{{ route('supervisor.observations.epoc', $observation) }}" class="mt-4 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold">Start review</a>
                </div>
                @endif
                </div>
                ...
                <div x-show="detailFilter==='all' || detailFilter==='post_conference'" x-transition.opacity class="section-card page-card overflow-hidden">
                    <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3">
                        ...
                        <div><h2 class="font-semibold text-gray-900 dark:text-white">Chat after class</h2><p class="text-xs text-gray-500 dark:text-gray-400">Feedback and next steps</p></div>
                    </div>
                    <div class="p-4">
                        @if($observation->postConference)
                            <div class="space-y-3">
                                ...
                                @if(!$observation->postConference->conference_date && !$observation->postConference->feedback && !$observation->postConference->ai_comparison)<p class="text-sm text-gray-500 dark:text-gray-400">Details will show up once the final chat is saved.</p>@endif
                            </div>
                        @else
                            <div class="text-center py-8 text-sm text-gray-500 dark:text-gray-400">No final chat recorded yet.</div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        @foreach($observation->evidence_files as $file)
                            ...
                            <a href="{{ asset('storage/'.$file['path']) }}" target="_blank" class="group flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-indigo-200 dark:hover:border-indigo-500/40 hover:bg-white dark:hover:bg-gray-700 transition-colors">
                                <span class="w-10 h-10 rounded-xl bg-white dark:bg-gray-800 border dark:border-gray-700 flex items-center justify-center shrink-0"><svg class="w-5 h-5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></span>
        ...
        <aside class="lg:col-span-4 space-y-4 lg:sticky lg:top-[124px] lg:max-h-[calc(100vh-140px)] lg:overflow-y-auto stepper-scroll lg:pb-1">
            ...
            <div class="page-card p-4">
                ...
                <dl class="space-y-3 text-sm">
                    ...
                    @if($observation->isTeacherObservation() && $observation->subject)<div class="flex justify-between gap-4"><dt class="text-gray-500 dark:text-gray-400">Subject</dt><dd class="font-medium truncate max-w-[150px] text-right">{{ $observation->subject }} · {{ $observation->grade_level_label }}</dd></div>@endif
            ...
            <div class="page-card p-4">
                ...
                <div class="mt-4 rounded-xl border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-800">
                    <p class="text-sm font-semibold">Review document</p><p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Official form filled in from the scores.</p>
