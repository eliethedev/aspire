                ...
                <div x-show="detailFilter==='all' || detailFilter==='observation' || detailFilter==='ratings'" x-transition.opacity class="section-card page-card overflow-hidden">
                    ...
                    <div class="p-5 space-y-2.5">
                        @foreach($observation->cotRatings as $rating)
                            @php $r=$rating->not_observed?null:$rating->rating; $rPct=$r?($r/6)*100:0; $rWrap=!$r?'border-gray-200 bg-gray-50':($r>=5?'border-emerald-200 bg-emerald-50/70':($r>=4?'border-blue-200 bg-blue-50/70':($r>=3?'border-amber-200 bg-amber-50/70':'border-red-200 bg-red-50/70'))); $rBadge=!$r?'bg-gray-100 text-gray-500':($r>=5?'bg-emerald-600 text-white':($r>=4?'bg-blue-600 text-white':($r>=3?'bg-amber-500 text-white':'bg-red-600 text-white'))); @endphp
                            <div class="rounded-xl border {{ $rWrap }} p-4">
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <div class="shrink-0 text-center"><div class="w-14 h-14 rounded-xl {{ $rBadge }} flex items-center justify-center text-sm font-bold shadow-sm">{{ $r?number_format($r,1):'NO' }}</div><p class="text-[10px] text-gray-400 mt-1">/ 6</p></div>
                                </div>
