        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="inline-flex items-center gap-1.5 text-xs bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full font-medium">
                ...
                <div class="flex items-center gap-0 mb-5">
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full {{ $planning && $planning->lesson_plan_file ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400' }} flex items-center justify-center">
                        ...
                        <span class="text-xs {{ $planning && $planning->lesson_plan_file ? 'text-emerald-700 font-semibold' : 'text-gray-400' }}">Uploaded</span>
                    </div>
                    <div class="flex-1 mx-2 h-px {{ $planning && $planning->lesson_plan_file ? 'bg-emerald-300' : 'bg-gray-200' }}"></div>
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full {{ $planning && $planning->ai_insights ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-400' }} flex items-center justify-center">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                        </div>
                        <span class="text-xs {{ $planning && $planning->ai_insights ? 'text-purple-700 font-semibold' : 'text-gray-400' }}">AI Analysis</span>
                    </div>
                    <div class="flex-1 mx-2 h-px {{ $planning && $planning->supervisor_notes ? 'bg-blue-300' : 'bg-gray-200' }}"></div>
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full {{ $planning && $planning->supervisor_notes ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400' }} flex items-center justify-center">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </div>
                        <span class="text-xs {{ $planning && $planning->supervisor_notes ? 'text-blue-700 font-semibold' : 'text-gray-400' }}">Review</span>
                    </div>
                </div>
                ...
                @if($planning && $planning->lesson_plan_file)
                    <div class="flex items-center justify-between bg-gradient-to-r from-blue-50 to-emerald-50 rounded-xl p-4 border border-blue-100">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-900">{{ preg_replace('/^\d+_/', '', basename($planning->lesson_plan_file)) }}</p>
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ asset('storage/' . $planning->lesson_plan_file) }}" target="_blank"
                               class="px-4 py-2 bg-white hover:bg-blue-50 text-blue-700 border border-blue-200 text-sm rounded-lg font-medium transition-colors inline-flex items-center gap-1.5">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Review AI-generated insights based on the lesson plan and previous observation data.</p>
                    </div>
                ...
                <div id="ai-insights-container">
                    @if($planning && $planning->ai_insights)
                        <div class="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-5 border border-purple-100">
                            ...
                            <div class="prose prose-sm max-w-none text-gray-700 whitespace-pre-wrap" id="ai-insights-text">{{ $planning->ai_insights }}</div>
                        </div>
                    @else
                        <div class="bg-gray-50 rounded-xl p-8 border-2 border-dashed border-gray-200 text-center" id="ai-insights-empty">
                            ...
                            <p class="text-sm font-medium text-gray-600">No AI insights available yet</p>
                            <p class="text-xs text-gray-400 mt-1 max-w-xs mx-auto">Click "Generate AI Insights" to analyze the lesson plan and generate recommendations based on previous observation data.</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="mt-4 flex items-center gap-2">
                    ...
                    <button type="button" id="clear-ai-insights-btn"
                            class="px-4 py-2.5 text-sm font-semibold text-red-600 hover:text-white bg-red-50 hover:bg-red-600 border border-red-200 hover:border-red-600 rounded-xl transition-all inline-flex items-center gap-1.5 {{ $planning?->ai_insights ? '' : 'hidden' }}">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Previous Observation Highlights</h2>
                        <p class="text-sm text-gray-500">Based on {{ $previousObservations->count() }} previous observation(s).</p>
                    </div>
                ...
                <div class="mb-4">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="text-sm font-semibold text-gray-700">Strengths</h3>
                    </div>
                    <div class="space-y-2">
                        @foreach($prevStrengths as $strength)
                            ...
                            <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-sm text-gray-700">{{ $strength->domain }}</span>
                                    <span class="text-xs font-semibold text-green-600">{{ number_format($strength->avg_rating, 1) }}/6</span>
                                </div>
                ...
                <div>
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="text-sm font-semibold text-gray-700">Areas for Improvement</h3>
                    </div>
                    <div class="space-y-2">
                        @foreach($prevWeaknesses as $weakness)
                            ...
                            <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-sm text-gray-700">{{ $weakness->domain }}</span>
                                    <span class="text-xs font-semibold text-red-600">{{ number_format($weakness->avg_rating, 1) }}/6</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-1.5">
                ...
                @if($prevStrengths->isEmpty() && $prevWeaknesses->isEmpty())
                    <div class="flex flex-col items-center py-8 text-center">
                        ...
                        <p class="text-sm text-gray-500">No detailed rating data available from previous observations.</p>
                    </div>
                @endif
            </div>
            @endif
