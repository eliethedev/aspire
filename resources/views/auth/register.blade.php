...
<body class="antialiased font-sans dark-bg min-h-screen flex items-center justify-center p-4 relative">
    ...
    <div class="w-full max-w-md glass-card rounded-2xl p-8 md:p-10 relative z-10">
        ...
        <div class="text-center mb-10">
            ...
            <h1 class="text-2xl font-bold text-white mb-2 tracking-tight">Join ASPIRE</h1>
            <p class="text-white/50 text-sm">Create your account to get started</p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-5">
            ...
            <!-- School Field -->
            <div>
                <label class="block text-sm font-semibold text-white/80 mb-2">School</label>
                <select name="school_id" class="dark-input w-full px-4 py-3 rounded-xl" required>
                    <option value="" class="bg-[#1a1a2e]">Select Your School</option>
                    @foreach(\App\Models\School::all() as $school)
                        <option value="{{ $school->id }}" {{ old('school_id') == $school->id ? 'selected' : '' }} class="bg-[#1a1a2e]">
                            {{ $school->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-semibold text-white/80 mb-2">Department</label>
                    <input 
                        type="text" 
                        name="department" 
                ...
                <div>
                    <label class="block text-sm font-semibold text-white/80 mb-2">Years of Service</label>
                    <input 
                        type="number" 
                ...
                <!-- Employee Number -->
                <div>
                    <label class="block text-sm font-semibold text-white/80 mb-2">Employee Number (TIN / DepEd ID) <span class="text-white/50 text-xs">- Recommended</span></label>
                    <input 
                        type="text" 
                        name="employee_number" 
                        value="{{ old('employee_number') }}"
                        class="dark-input w-full px-4 py-3 rounded-xl"
                        placeholder="e.g. 123456789 or DEPED-2023-001"
                    >
                ...
                <!-- Mobile Number -->
                <div>
                    <label class="block text-sm font-semibold text-white/80 mb-2">Mobile Number</label>
                    <input 
                        type="tel" 
                        name="mobile_number" 
                ...
                <!-- PRC License Number -->
                <div>
                    <label class="block text-sm font-semibold text-white/80 mb-2">PRC License Number <span class="text-white/50 text-xs">- Optional</span></label>
                    <input 
                        type="text" 
                ...
                <div>
                    <label class="block text-sm font-semibold text-white/80 mb-2">Position / Grade Level <span class="text-white/50 text-xs">- Recommended</span></label>
                    <select name="position" class="dark-input w-full px-4 py-3 rounded-xl">
                        <option value="" class="bg-[#1a1a2e]">Select Position</option>
                        <option value="Teacher I" {{ old('position') == 'Teacher I' ? 'selected' : '' }} class="bg-[#1a1a2e]">Teacher I</option>
                        <option value="Teacher II" {{ old('position') == 'Teacher II' ? 'selected' : '' }} class="bg-[#1a1a2e]">Teacher II</option>
                        <option value="Teacher III" {{ old('position') == 'Teacher III' ? 'selected' : '' }} class="bg-[#1a1a2e]">Teacher III</option>
                        <option value="Master Teacher I" {{ old('position') == 'Master Teacher I' ? 'selected' : '' }} class="bg-[#1a1a2e]">Master Teacher I</option>
                        <option value="Master Teacher II" {{ old('position') == 'Master Teacher II' ? 'selected' : '' }} class="bg-[#1a1a2e]">Master Teacher II</option>
                        <option value="Head Teacher" {{ old('position') == 'Head Teacher' ? 'selected' : '' }} class="bg-[#1a1a2e]">Head Teacher</option>
                        <option value="Principal I" {{ old('position') == 'Principal I' ? 'selected' : '' }} class="bg-[#1a1a2e]">Principal I</option>
                        <option value="Principal II" {{ old('position') == 'Principal II' ? 'selected' : '' }} class="bg-[#1a1a2e]">Principal II</option>
                        <option value="Principal III" {{ old('position') == 'Principal III' ? 'selected' : '' }} class="bg-[#1a1a2e]">Principal III</option>
                        <option value="Other" {{ old('position') == 'Other' ? 'selected' : '' }} class="bg-[#1a1a2e]">Other</option>
                    </select>
                    @error('position')
                        <p class="mt-2 text-sm text-rose-400 flex items-center">
                            <svg class="w-4 h-4 mr-1.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
    ...
    <script>
        ...
        // Role is automatically set to 'teacher' for all self-registrations
        // Higher roles can only be assigned by authorized administrators
    </script>
</body>
</html>
